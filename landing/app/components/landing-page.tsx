    ...
    <>
      ...
      >
        <div className="mx-auto flex max-w-7xl items-center justify-between px-3 py-3 sm:px-8 lg:px-12 lg:py-4">
          <a href="#hero" className="flex min-w-0 items-center gap-3 text-lg font-semibold tracking-tight">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-slate-950 text-white shadow-lg shadow-sky-500/20">
              <Bot className="h-5 w-5" />
            </div>
            <span className="truncate text-[0.95rem] sm:text-[1rem]">AR AI Chat</span>
          </a>
...
type BookDemoSectionProps = {
  ...
  setDemoForm: Dispatch<SetStateAction<DemoFormState>>;
  handleDemoSubmit: (event: FormEvent<HTMLFormElement>) => void;
        ...
        <div className="grid gap-8 lg:grid-cols-[0.95fr_1.05fr] lg:gap-10">
          ...
          <form onSubmit={handleDemoSubmit} className="rounded-[30px] border border-slate-200 bg-white/95 p-6 text-slate-900 shadow-[0_30px_90px_-35px_rgba(15,23,42,0.45)] sm:p-7 lg:p-8">
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              <FormField label="Full Name" id="demo-name" name="name" value={demoForm.name} onChange={(event) => setDemoForm((current) => ({ ...current, name: event.target.value }))} placeholder="Alex Morgan" required inputClassName="h-12 rounded-2xl border-slate-200 bg-slate-50 px-4 text-sm" />
              <FormField label="Work Email" id="demo-email" type="email" name="email" value={demoForm.email} onChange={(event) => setDemoForm((current) => ({ ...current, email: event.target.value }))} placeholder="alex@company.com" required inputClassName="h-12 rounded-2xl border-slate-200 bg-slate-50 px-4 text-sm" />
              <FormField label="Phone Number" id="demo-phone" type="tel" name="phone" value={demoForm.phone} onChange={(event) => setDemoForm((current) => ({ ...current, phone: event.target.value }))} placeholder="+1 555 123 4567" inputClassName="h-12 rounded-2xl border-slate-200 bg-slate-50 px-4 text-sm" />
              <FormField label="Company" id="demo-company" name="company" value={demoForm.company} onChange={(event) => setDemoForm((current) => ({ ...current, company: event.target.value }))} placeholder="Northwind Labs" required inputClassName="h-12 rounded-2xl border-slate-200 bg-slate-50 px-4 text-sm" />
              <FormField label="Company Website" id="demo-website" type="url" name="website" value={demoForm.website} onChange={(event) => setDemoForm((current) => ({ ...current, website: event.target.value }))} placeholder="https://example.com" inputClassName="h-12 rounded-2xl border-slate-200 bg-slate-50 px-4 text-sm" />
              <div className="space-y-2">
                <label htmlFor="demo-industry" className="text-sm font-medium text-slate-700">Industry</label>
                <select id="demo-industry" value={demoForm.industry} onChange={(event) => setDemoForm((current) => ({ ...current, industry: event.target.value }))} className="h-12 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 text-sm text-slate-900 outline-none transition focus:border-sky-500 focus:ring-2 focus:ring-sky-100">
              ...
              <div className="space-y-2">
                ...
                <select id="demo-team-size" value={demoForm.teamSize} onChange={(event) => setDemoForm((current) => ({ ...current, teamSize: event.target.value }))} className="h-12 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 text-sm text-slate-900 outline-none transition focus:border-sky-500 focus:ring-2 focus:ring-sky-100">
        ...
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:gap-10">
          ...
          <form onSubmit={handleContactSubmit} className="rounded-[30px] border border-slate-200 bg-slate-950 p-7 text-white shadow-[0_30px_90px_-35px_rgba(15,23,42,0.45)] sm:p-8 lg:p-10">
            ...
            <div className="mt-5 space-y-4">
              <FormField label="Name" id="contact-name" name="name" value={contactForm.name} onChange={(event) => setContactForm((current) => ({ ...current, name: event.target.value }))} placeholder="Jamie Lee" required inputClassName="h-12 rounded-2xl border-slate-700 bg-slate-900/70 px-4 text-sm text-white" />
              <FormField label="Email" id="contact-email" type="email" name="email" value={contactForm.email} onChange={(event) => setContactForm((current) => ({ ...current, email: event.target.value }))} placeholder="jamie@company.com" required inputClassName="h-12 rounded-2xl border-slate-700 bg-slate-900/70 px-4 text-sm text-white" />
              <FormField label="Subject" id="contact-subject" name="subject" value={contactForm.subject} onChange={(event) => setContactForm((current) => ({ ...current, subject: event.target.value }))} placeholder="Product question or partnership" inputClassName="h-12 rounded-2xl border-slate-700 bg-slate-900/70 px-4 text-sm text-white" />
              <FormField label="Message" id="contact-message" name="message" textarea value={contactForm.message} onChange={(event) => setContactForm((current) => ({ ...current, message: event.target.value }))} placeholder="Tell us about your goals, timeline, or support needs." required inputClassName="min-h-28 rounded-2xl border-slate-700 bg-slate-900/70 px-4 py-3 text-sm text-white" />
            </div>
