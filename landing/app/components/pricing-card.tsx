  ...
  return (
    <div
      className={`relative flex h-full flex-col overflow-hidden rounded-[30px] border p-8 shadow-[0_30px_80px_-40px_rgba(15,23,42,0.35)] transition duration-300 hover:-translate-y-2 ${
        highlighted
          ? "border-sky-500 bg-slate-950 text-white shadow-[0_35px_100px_-30px_rgba(14,165,233,0.35)]"
    ...
    >
      ...
      <div className="flex items-center justify-between">
        ...
        {highlighted ? (
          <span className="rounded-full border border-sky-400/40 bg-sky-500/10 px-3 py-1 text-sm font-medium text-sky-300">
            Most Popular
          </span>
        ) : null}
      </div>
      ...
      <ul className="mt-8 flex-1 space-y-3">
        {features.map((feature) => (
          <li key={feature} className="flex items-start gap-3 text-sm">
            <Check className={`mt-0.5 h-4 w-4 ${highlighted ? "text-sky-400" : "text-sky-600"}`} />
            <span>{feature}</span>
          </li>
        ))}
      </ul>
      <button
        type="button"
        className={`mt-8 w-full rounded-full px-5 py-3 text-sm font-semibold transition duration-200 active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500 focus-visible:ring-offset-2 ${
          highlighted
